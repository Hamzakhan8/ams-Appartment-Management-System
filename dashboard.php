...
$result_floor = mysqli_query($link,"SELECT count(branch_id) as total_floor FROM tblbranch ");
if($row_floor_total = mysqli_fetch_array($result_floor)){
	$total_floor = $row_floor_total['total_floor'];
}
//unit count
$result_unit = mysqli_query($link,"SELECT COUNT(unit.uid) AS total_unit
FROM tbl_add_unit AS unit 
WHERE EXISTS (
    SELECT 1 
    FROM tbl_add_rent AS rent 
    WHERE unit.uid = rent.r_unit_no
) ");
if($row_unit_total = mysqli_fetch_array($result_unit)){
	$total_unit = $row_unit_total['total_unit'];
}
$result_units = mysqli_query($link, "SELECT COUNT(unit.uid) AS total_units 
                                     FROM tbl_add_unit AS unit 
                                     WHERE NOT EXISTS (
                                         SELECT 1 
                                         FROM tbl_add_rent AS rent 
...
if ($row_units_total = mysqli_fetch_array($result_units)) {
    $total_units = $row_units_total['total_units'];
}
//owner count
$result_owner = mysqli_query($link,"SELECT count(ownid) as total_owner FROM tbl_add_owner ");
if($row_owner_total = mysqli_fetch_array($result_owner)){
	$total_owner = $row_owner_total['total_owner'];
}
//rented count
$result_rented = mysqli_query($link,"SELECT count(rid) as total_rent FROM tbl_add_rent ");
...
<section class="content-header">
  ...
  <!-- /.row start -->
  <div class="row home_dash_box" style="margin-top:16px;">
    <div class="col-lg-4 col-xs-6">
      <div class="small-box bg-primary box-size" >
        <div class="inner">
    ...
    <div class="col-lg-4 col-xs-6">
      <div class="small-box bg-yellow box-size" >
        <div class="inner">
    ...
    <div class="col-lg-4 col-xs-6">
      <div class="small-box bg-red box-size" >
        <div class="inner">
    ...
    <div class="col-lg-6 col-xs-6">
      <div class="small-box bg-green box-size" >
        <div class="inner">
          <h3><?php echo $total_units; ?></h3>
          <p>Total Availible Units</p>
        </div>
        <div class="icon"> <img height="80" width="80" src="img/office-building-icon.png"></a> </div>
        <!-- <a href="<?php echo WEB_URL; ?>unit/unitlist.php" class="small-box-footer"><?php echo $_data['dashboard_more_info']; ?> <i class="fa fa-arrow-circle-right"></i></a> -->
       </div>
    </div>
  </div>
