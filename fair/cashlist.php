        ...
        <table class="table sakotable table-bordered table-striped dt-responsive">
          ...
          <tbody>
            <?php
              ...
              LEFT JOIN tbl_add_owner ao ON tb.building_make_year = ao.branch_id 
            ...
            <?php
                  $rid = $row['rid'];
                  $monthCondition = " AND MONTH(date) = MONTH(CURDATE())";
                  $resxx = mysqli_query($link, "SELECT * FROM tbl_emi_payment WHERE rid = $rid $monthCondition");
                  $emiAmount = 0;
            ...
            <td> <?php
                        $resxx = mysqli_query($link, "SELECT * FROM tbl_emi_payment 
                                                        WHERE rid = " . $row['rid'] . " 
                        ...
                        if ($resxx) {
                            while ($pay = mysqli_fetch_array($resxx)) {
                                if ($pay['status'] == 2) {
                                    $query = "SELECT `date` FROM `tbl_particle_payments` WHERE ep_id = " . $pay['id'];
                                    $resx = mysqli_query($link, $query);

                                    if ($resx) {
                                        while ($amountResult = mysqli_fetch_array($resx)) {
                                            $dates[] = $amountResult['date'];
                                        }
                                    }
                                }
                            }
                        ...
                        echo (!empty($dates) ? implode(', ', $dates) : '');
                   ?>
